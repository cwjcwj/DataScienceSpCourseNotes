...
* `head(rownames(a),3)` = returns first three items in first argument
...
* `library(packagename)` = load package
    * ***Note**: `packagename` is **not** quoted *
* `search()` = see all loaded packages
* `ls("package:packagename")` = see functions in loaded package
...
* **Predictive analysis** = use data on one object to predict values for another (Credit scores, SAT test results)
